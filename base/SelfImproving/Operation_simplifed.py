...
if __name__ == "__main__":
    ...
    #     for __ in range(m)
    # ]
    from PointDistribution import generate_clustered_points_trunc
    k=3
    train_samples = [
        generate_clustered_points_trunc(n, k,
                                          x_range=(5, 15),
                                          y_range=(0, 7.5),
                                          std_dev=0.8) for _ in range(n)
        for __ in range(m)
    ]
    state = training_phase(train_samples, n)
    # 生成一个新的测试实例
    # I_new = [(random.uniform(5, 15), random.uniform(0, 7.5)) for _ in range(n)]
    I_new = generate_clustered_points_trunc(n, k,
                                          x_range=(5, 15),
                                          y_range=(0, 7.5),
                                          std_dev=0.8)
