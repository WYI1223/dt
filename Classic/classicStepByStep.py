...
class Triangulation:
    ...
    def plot(self, pause_time=0.1):
        plt.clf()  # Clear the figure to avoid overlaying plots
        ...
        plt.xlim(0, 100)
        plt.ylim(0, 100)
        plt.pause(pause_time)  # Pause to show plot

def main():
    np.random.seed(42)  # Set the random seed for reproducibility
    points = np.random.rand(100, 2) * 20  # generate 100 random points
    triangulation = Triangulation()
    plt.figure(figsize=(8, 8))
    for point in points:
        triangulation.add_point(point)
        triangulation.plot(pause_time=0.5)  # Set a suitable delay, e.g., 0.5 seconds
